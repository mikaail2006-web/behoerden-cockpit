...
const INTEGRATION_CHECKLIST_STORAGE_KEY = "behoerden-cockpit-integration-checklist";
const INTEGRATION_TEST_STORAGE_KEY = "behoerden-cockpit-integration-test-run";
...
const els = {
  ...
  integrationChecklist: document.querySelector("#integrationChecklist"),
  integrationReadiness: document.querySelector("#integrationReadiness"),
  integrationTestCard: document.querySelector("#integrationTestCard"),
  integrationTestChecklist: document.querySelector("#integrationTestChecklist"),
  integrationTestStatus: document.querySelector("#integrationTestStatus"),
  integrationTestChannelInput: document.querySelector("#integrationTestChannelInput"),
  integrationTestRecipientInput: document.querySelector("#integrationTestRecipientInput"),
  integrationRecipientHint: document.querySelector("#integrationRecipientHint"),
  integrationTestOptInInput: document.querySelector("#integrationTestOptInInput"),
  integrationTestConfigStatus: document.querySelector("#integrationTestConfigStatus"),
  integrationMessagePreview: document.querySelector("#integrationMessagePreview"),
  integrationPayloadPreview: document.querySelector("#integrationPayloadPreview"),
  integrationResultCheck: document.querySelector("#integrationResultCheck"),
  integrationResultNoteInput: document.querySelector("#integrationResultNoteInput"),
  integrationResultStatus: document.querySelector("#integrationResultStatus"),
  integrationFlowGuide: document.querySelector("#integrationFlowGuide"),
  integrationFlowStatus: document.querySelector("#integrationFlowStatus"),
  ...
  resetIntegrationChecklistButton: document.querySelector("#resetIntegrationChecklistButton"),
...
function readIntegrationChecklist() {
  try {
    return JSON.parse(getStoredValue(INTEGRATION_CHECKLIST_STORAGE_KEY) || "{}");
  } catch {
    return {};
  }
}

function writeIntegrationChecklist(values) {
  setStoredValue(INTEGRATION_CHECKLIST_STORAGE_KEY, JSON.stringify(values));
}

function integrationDoneCount(values = readIntegrationChecklist()) {
  return ["whatsappProvider", "whatsappTemplate", "recipientOptIn", "kiEndpoint", "n8nCredentials", "integrationTest"].filter((key) => values[key]).length;
}

function readIntegrationTestRun() {
  try {
    return JSON.parse(getStoredValue(INTEGRATION_TEST_STORAGE_KEY) || "{}");
  } catch {
    return {};
  }
}

function writeIntegrationTestRun(values) {
  setStoredValue(INTEGRATION_TEST_STORAGE_KEY, JSON.stringify(values));
}

function integrationTestDoneCount(values = readIntegrationTestRun()) {
  return ["upload", "drive", "sheet", "analysis", "reminder"].filter((key) => values[key]).length;
}

function integrationFlowDoneCount(values = readIntegrationTestRun()) {
  return ["payloadCopied", "webhookRun", "resultsChecked", "protocolReady"].filter((key) => values[key]).length;
}

function integrationResultDoneCount(values = readIntegrationTestRun()) {
  return ["driveResult", "sheetResult", "analysisResult", "reminderResult"].filter((key) => values[key]).length;
}

function isIntegrationTestRecipientReady(values = readIntegrationTestRun()) {
  const validation = integrationRecipientValidation(values);
  return validation.valid && Boolean(values.optIn || validation.channel === "audit");
}

function integrationRecipientValidation(values = readIntegrationTestRun()) {
  const channel = values.channel || "whatsapp";
  const recipient = String(values.recipient || "").trim();
  if (channel === "audit") return { channel, valid: true, message: "Audit-Log ohne Empfaenger" };
  if (!recipient) return { channel, valid: false, message: "Empfaenger fehlt" };
  if (channel === "email") {
    const valid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recipient);
    return { channel, valid, message: valid ? "E-Mail-Format passt" : "E-Mail-Format pruefen" };
  }
  const valid = /^\+[1-9]\d{7,14}$/.test(recipient.replace(/\s+/g, ""));
  return { channel, valid, message: valid ? "Telefonformat passt" : "Telefonformat mit +Laendercode pruefen" };
}

function integrationRecipientFormatHint(channel = "whatsapp") {
  if (channel === "email") {
    return { placeholder: "test@example.de", hint: "E-Mail erwartet eine Adresse wie test@example.de." };
  }
  if (channel === "audit") {
    return { placeholder: "kein Empfaenger notwendig", hint: "Nur Audit-Log nutzt keinen externen Empfaenger." };
  }
  return { placeholder: "+49123456789", hint: "WhatsApp erwartet Telefonnummer mit +Laendercode, ohne Leerzeichen." };
}
...
function maskRecipient(value = "") {
  ...
  const visible = cleanValue.replace(/\s+/g, "");
...
function integrationTestMessagePreview(values = readIntegrationTestRun()) {
  const nextDeadline = sortByUrgency(activeItems(state.deadlines).filter((item) => !item.done), "date", "escalation")[0];
  const area = nextDeadline?.area || "EM-Rente";
  const title = nextDeadline?.title || "Testfrist prüfen";
  const date = nextDeadline?.date || dateAfter(7);
  const nextStep = nextDeadline?.nextStep || "Unterlagen prüfen und Rueckmeldung vorbereiten";
  const channel = values.channel || "whatsapp";
  if (channel === "audit") {
    return `TEST-AUDIT: ${area} - ${title} bis ${date}. Naechster Schritt: ${nextStep}. Kein Versand.`;
  }
  if (channel === "email") {
    return `Betreff: TEST Erinnerung ${title}\n\nDies ist ein unkritischer Testlauf im Behoerden-Cockpit.\nBereich: ${area}\nFrist: ${date}\nNaechster Schritt: ${nextStep}\n\nBitte nicht mit echten Gesundheitsdaten verwenden.`;
  }
  return `TEST Behörden-Cockpit: ${title} (${area}) bis ${date}. Naechster Schritt: ${nextStep}. Nur Testversand an eigene Nummer.`;
}

function integrationTestPayload(values = readIntegrationTestRun()) {
  const nextDeadline = sortByUrgency(activeItems(state.deadlines).filter((item) => !item.done), "date", "escalation")[0];
  return {
    ...
    recipientMasked: maskRecipient(values.recipient),
    optInConfirmed: Boolean(values.optIn),
    caseArea: nextDeadline?.area || "EM-Rente",
    deadlineTitle: nextDeadline?.title || "Testfrist prüfen",
    deadlineDate: nextDeadline?.date || dateAfter(7),
    nextStep: nextDeadline?.nextStep || "Unterlagen prüfen und Rueckmeldung vorbereiten",
    message: integrationTestMessagePreview(values),
    safety: "Keine echten Gesundheitsdaten im Testpayload verwenden."
  };
}

function integrationPayloadSafetyItems(values = readIntegrationTestRun()) {
  const payload = integrationTestPayload(values);
  const validation = integrationRecipientValidation(values);
  return [
    ["mode", payload.mode === "test", "Testmodus aktiv"],
    ["recipient", payload.recipientMasked === "-" || payload.recipientMasked.includes("***"), "Empfaenger maskiert"],
    ["recipientFormat", validation.valid, validation.message],
    ["safety", payload.safety.includes("Keine echten Gesundheitsdaten"), "Sicherheitshinweis enthalten"],
    ["optin", payload.channel === "audit" || payload.optInConfirmed, "Opt-in fuer Versandkanal bestaetigt"]
  ];
}

function isFullReadinessReady() {
  return readinessCounts().totalDone === readinessCounts().totalRequired;
}
...
function logReadinessOverride({ fileName, area, type }) {
  const { preflightDone, opsDone, integrationDone, totalDone, totalRequired } = readinessCounts();
  const auditItem = {
    id: `LOG-${String(state.auditLog.length + 1).padStart(3, "0")}`,
    timestamp: nowTimestamp(),
    action: "Upload trotz offener Echtdaten-Freigabe bestätigt",
    object: fileName,
    area,
    details: `${type} wurde trotz offener Echtdaten-Freigabe gestartet. Stand: ${totalDone}/${totalRequired}; Preflight ${preflightDone}/6, Betrieb ${opsDone}/6, Integrationen ${integrationDone}/6.`,
    source: "App",
    level: "warning"
  };
  state.auditLog.unshift(auditItem);
  renderActivity();
  persistManualEntry("audit", auditItem).then((saved) => saved && showToast("Echtdaten-Ausnahme im Google Sheet protokolliert"));
}
...
function updateDocumentCheckStatus() {
  ...
  const readiness = readinessCounts();
  const fullReady = readiness.totalDone === readiness.totalRequired;
  ...
  if (els.uploadPreflightStatus) {
    els.uploadPreflightStatus.textContent = fullReady
      ? "Echte Dokumente: Freigabe vollständig. Echtdaten-Upload kontrolliert möglich."
      : `Echte Dokumente: noch gesperrt, Freigabe ${readiness.totalDone}/${readiness.totalRequired}. Preflight ${preflightDone}/6.`;
    els.uploadPreflightStatus.className = `upload-preflight-status ${fullReady ? "ready" : ""}`;
    els.uploadPreflightStatus.disabled = fullReady;
  }
  if (els.documentCheckSubmitButton) {
    els.documentCheckSubmitButton.innerHTML = fullReady
      ? `<span data-icon="sparkles"></span>Echtdaten prüfen`
      : `<span data-icon="shield-alert"></span>Mit Freigabe-Hinweis prüfen`;
  }
...
function openSheetSyncFallbackCount() {
  return state.auditLog.filter((item) => item.action === "Sheet-Speicherung offen").length;
}

function readinessCounts() {
  const preflightDone = preflightDoneCount();
  const opsDone = opsDoneCount();
  const integrationDone = integrationDoneCount();
  const totalDone = preflightDone + opsDone + integrationDone;
  return { preflightDone, opsDone, integrationDone, totalDone, totalRequired: 18 };
}

function readinessNextStep({ preflightDone, opsDone, integrationDone, totalDone, totalRequired }) {
  if (totalDone === totalRequired) return "Nächster Schritt: erstes echtes Dokument einzeln kontrolliert hochladen.";
  if (preflightDone < 6) return "Nächster Schritt: Setup > Sicherheit öffnen und Echte-Daten-Preflight vervollständigen.";
  if (opsDone < 6) return "Nächster Schritt: Setup > Betriebsroutine öffnen und Backup/Wiederherstellung prüfen.";
  if (integrationDone < 6) return "Nächster Schritt: Automationen öffnen und Produktiv-Integrationen vervollständigen.";
  return "Nächster Schritt: Freigabebericht herunterladen und Echtdaten-Upload kontrolliert starten.";
}

function readinessNextTarget({ preflightDone, opsDone, integrationDone, totalDone, totalRequired }) {
  if (totalDone === totalRequired) return { label: "Upload öffnen", view: "check", focus: "documentCheckForm", toast: "Upload-Check geöffnet" };
  if (preflightDone < 6) return { label: "Sicherheit öffnen", view: "setup", focus: "securitySetupSection", toast: "Setup-Sicherheit geöffnet" };
  if (opsDone < 6) return { label: "Betrieb öffnen", view: "setup", focus: "opsSetupSection", toast: "Betriebsroutine geöffnet" };
  if (integrationDone < 6) return { label: "Automationen öffnen", view: "automation", focus: "integrationSetupSection", toast: "Produktiv-Integrationen geöffnet" };
  return { label: "Dashboard öffnen", view: "dashboard", focus: "readinessBanner", toast: "Dashboard geöffnet" };
}

function renderReadinessBanner() {
  if (!els.readinessBanner) return;
  const counts = readinessCounts();
  const { preflightDone, opsDone, integrationDone, totalDone, totalRequired } = counts;
  const ready = totalDone === totalRequired;
  const nextTarget = readinessNextTarget(counts);
  els.readinessBanner.className = `readiness-banner ${ready ? "ready" : ""}`;
  els.readinessBanner.innerHTML = `
    <div>
      <strong>${ready ? "Echte Daten freigegeben" : "Echte Daten noch gesperrt"}</strong>
      <span>${ready ? "Preflight, Betrieb und Integrationen sind vollständig bestätigt." : "Vor echten sensiblen Dokumenten fehlen noch Freigabepunkte."}</span>
      <span class="readiness-next-step">${readinessNextStep(counts)}</span>
      <div class="readiness-actions">
        <button class="text-button readiness-next-button" type="button" id="openReadinessNextStepButton" data-readiness-target="${nextTarget.view}" data-readiness-focus="${nextTarget.focus}" data-readiness-toast="${nextTarget.toast}"><span data-icon="arrow-right"></span>${nextTarget.label}</button>
        <button class="text-button readiness-report-button" type="button" id="downloadReadinessReportButton"><span data-icon="download"></span>Freigabebericht</button>
      </div>
    </div>
    <div class="readiness-score">
      <b>${totalDone}/${totalRequired}</b>
      <span>Preflight ${preflightDone}/6 · Betrieb ${opsDone}/6 · Integrationen ${integrationDone}/6</span>
    </div>
  `;
}

function checklistStatusLines(title, labels, values) {
  return [
    title,
    ...labels.map(([key, label]) => `- ${values[key] ? "[x]" : "[ ]"} ${label}`)
  ].join("\n");
}
...
function renderQualityStrip() {
  ...
  const opsDone = opsDoneCount();
  const integrationDone = integrationDoneCount();
  const sheetSyncOpen = openSheetSyncFallbackCount();
  const checks = [
    {
    ...
    {
      label: "Integrationen",
      value: `${integrationDone}/6`,
      detail: integrationDone === 6 ? "produktivreif" : "Credentials offen",
      level: integrationDone === 6 ? "ok" : "warning",
      target: "automation",
      search: ""
    },
    {
      label: "Sheet-Sync",
      value: sheetSyncOpen,
      detail: sheetSyncOpen ? "Protokollierung prüfen" : "keine offenen",
      level: sheetSyncOpen ? "warning" : "ok",
      target: "activity",
      search: "Sheet-Speicherung offen"
    }
  ];
...
function renderActivity() {
  ...
  els.activityList.innerHTML = entries.map((item) => `
    <article class="activity-item ${item.level || ""}">
      ...
      ${item.action === "Sheet-Speicherung offen" ? `
        <div class="activity-actions">
          <button class="text-button activity-action" type="button" data-retry-sheet-sync="${item.id}"><span data-icon="refresh-cw"></span>Erneut senden</button>
          <button class="text-button activity-action" type="button" data-resolve-sheet-sync="${item.id}"><span data-icon="check"></span>Nachprüfung notieren</button>
        </div>
      ` : ""}
    </article>
...
async function retrySheetSyncFallback(id) {
  const item = state.auditLog.find((entry) => entry.id === id);
  if (!item || item.action !== "Sheet-Speicherung offen") return;
  const saved = await persistManualEntry("audit", item);
  if (!saved) {
    showToast("Sheet-Sync weiterhin offen");
    return;
  }
  item.action = "Sheet-Speicherung nachgesendet";
  item.level = "info";
  item.details = `${item.details} Erneut erfolgreich ans Sheet gesendet am ${nowTimestamp()}.`;
  renderActivity();
  renderQualityStrip();
  renderMetrics();
  showToast("Sheet-Sync erneut gesendet");
}

function resolveSheetSyncFallback(id) {
  const item = state.auditLog.find((entry) => entry.id === id);
  if (!item || item.action !== "Sheet-Speicherung offen") return;
  item.action = "Sheet-Speicherung nachgeprüft";
  item.level = "info";
  item.details = `${item.details} Nachpruefung lokal am ${nowTimestamp()} notiert.`;
  renderActivity();
  renderQualityStrip();
  renderMetrics();
  persistManualEntry("audit", item).then((saved) => saved && showToast("Nachprüfung im Google Sheet protokolliert"));
  showToast("Sheet-Sync-Nachprüfung notiert");
}
...
function renderIntegrationChecklist() {
  if (!els.integrationChecklist) return;
  const values = readIntegrationChecklist();
  const doneCount = integrationDoneCount(values);
  els.integrationChecklist.querySelectorAll("[data-integration-item]").forEach((input) => {
    input.checked = Boolean(values[input.dataset.integrationItem]);
  });
  if (els.integrationReadiness) {
    els.integrationReadiness.textContent = doneCount === 6
      ? "Produktiv-Integrationen vollständig bestätigt."
      : `Integrationen offen: ${doneCount}/6 Punkten erledigt.`;
    els.integrationReadiness.className = doneCount === 6 ? "ready" : "";
  }
}

function renderIntegrationTestRun() {
  if (!els.integrationTestCard || !els.integrationTestStatus) return;
  const values = readIntegrationTestRun();
  const recipientReady = isIntegrationTestRecipientReady(values);
  if (!recipientReady && values.reminder) {
    values.reminder = false;
    delete values.completedAt;
    writeIntegrationTestRun(values);
  }
  const ready = Boolean(values.completedAt);
  const prepared = Boolean(values.preparedAt);
  const doneCount = integrationTestDoneCount(values);
  els.integrationTestCard.className = `integration-test-card ${ready ? "ready" : prepared ? "prepared" : ""}`;
  els.integrationTestStatus.textContent = ready
    ? `Protokolliert am ${values.completedAt}.`
    : prepared
      ? `Testlauf offen: ${doneCount}/5 Punkten erledigt.`
      : `Noch nicht protokolliert: ${doneCount}/5 Punkten erledigt.`;
  els.integrationTestChecklist?.querySelectorAll("[data-integration-test-item]").forEach((input) => {
    const isReminderInput = input.dataset.integrationTestItem === "reminder";
    input.disabled = isReminderInput && !recipientReady;
    input.checked = Boolean(values[input.dataset.integrationTestItem]);
    input.closest("label")?.classList.toggle("done", input.checked);
    input.closest("label")?.classList.toggle("locked", input.disabled);
  });
  if (els.integrationTestChannelInput) els.integrationTestChannelInput.value = values.channel || "whatsapp";
  const recipientHint = integrationRecipientFormatHint(values.channel || "whatsapp");
  if (els.integrationTestRecipientInput) {
    els.integrationTestRecipientInput.value = values.recipient || "";
    els.integrationTestRecipientInput.placeholder = recipientHint.placeholder;
    els.integrationTestRecipientInput.disabled = (values.channel || "whatsapp") === "audit";
  }
  if (els.integrationRecipientHint) els.integrationRecipientHint.textContent = recipientHint.hint;
  if (els.integrationTestOptInInput) els.integrationTestOptInInput.checked = Boolean(values.optIn);
  if (els.integrationTestConfigStatus) {
    const channelLabel = { whatsapp: "WhatsApp", email: "E-Mail", audit: "Audit-Log" }[values.channel || "whatsapp"];
    const validation = integrationRecipientValidation(values);
    els.integrationTestConfigStatus.textContent = recipientReady
      ? `Testversand vorbereitet: ${channelLabel}.`
      : `Testversand offen: ${validation.message}; Opt-in prüfen.`;
    els.integrationTestConfigStatus.className = recipientReady ? "ready" : "";
  }
  if (els.integrationMessagePreview) {
    els.integrationMessagePreview.innerHTML = `
      <div>
        <strong>Testnachricht-Vorschau</strong>
        <button class="mini-button" type="button" id="copyIntegrationMessageButton" title="Testnachricht kopieren"><span data-icon="copy"></span></button>
      </div>
      <pre>${escapeHtml(integrationTestMessagePreview(values))}</pre>
      <span>${recipientReady ? `Ziel: ${maskRecipient(values.recipient)}.` : "Ziel: noch nicht freigegeben."}</span>
    `;
    els.integrationMessagePreview.className = `integration-message-preview ${recipientReady ? "ready" : ""}`;
  }
  if (els.integrationPayloadPreview) {
    const safetyItems = integrationPayloadSafetyItems(values);
    const safetyReady = safetyItems.every(([, ok]) => ok);
    els.integrationPayloadPreview.innerHTML = `
      <div>
        <strong>n8n-Testpayload-Vorschau</strong>
        <span>${safetyReady ? "sicher" : "prüfen"}</span>
      </div>
      <pre>${escapeHtml(JSON.stringify(integrationTestPayload(values), null, 2))}</pre>
      <div class="payload-safety-list">
        ${safetyItems.map(([key, ok, label]) => `<span class="${ok ? "ok" : "warning"}" data-payload-check="${key}">${ok ? "OK" : "!"} ${label}</span>`).join("")}
      </div>
    `;
    els.integrationPayloadPreview.className = `integration-payload-preview ${safetyReady ? "ready" : ""}`;
  }
  if (els.integrationResultCheck) {
    els.integrationResultCheck.querySelectorAll("[data-result-item]").forEach((input) => {
      input.checked = Boolean(values[input.dataset.resultItem]);
      input.closest("label")?.classList.toggle("done", input.checked);
    });
  }
  if (els.integrationResultNoteInput) {
    els.integrationResultNoteInput.value = values.resultNote || "";
  }
  if (els.integrationResultStatus) {
    const resultDone = integrationResultDoneCount(values);
    els.integrationResultStatus.textContent = resultDone === 4
      ? "n8n-Ergebnis vollständig geprüft."
      : `n8n-Ergebnis offen: ${resultDone}/4.`;
    els.integrationResultStatus.className = resultDone === 4 ? "ready" : "";
  }
  if (els.integrationFlowGuide) {
    els.integrationFlowGuide.querySelectorAll("[data-flow-item]").forEach((input) => {
      input.checked = Boolean(values[input.dataset.flowItem]);
      input.closest("label")?.classList.toggle("done", input.checked);
    });
  }
  if (els.integrationFlowStatus) {
    const flowDone = integrationFlowDoneCount(values);
    els.integrationFlowStatus.textContent = flowDone === 4
      ? "n8n-Testablauf vollständig bestätigt."
      : `n8n-Testablauf offen: ${flowDone}/4.`;
    els.integrationFlowStatus.className = flowDone === 4 ? "ready" : "";
  }
  const markButton = document.querySelector("#markIntegrationTestButton");
  if (markButton) markButton.disabled = doneCount < 5 || ready;
}
...
function exportReadinessReport() {
  const readiness = readinessCounts();
  const preflight = readPreflightChecklist();
  const ops = readOpsChecklist();
  const integrations = readIntegrationChecklist();
  const ready = readiness.totalDone === readiness.totalRequired;
  const report = [
    "Behörden-Cockpit Echtdaten-Freigabebericht",
    "",
    `Erstellt: ${nowTimestamp()}`,
    `Status: ${ready ? "Freigegeben" : "Gesperrt"}`,
    `Gesamt: ${readiness.totalDone}/${readiness.totalRequired}`,
    `Preflight: ${readiness.preflightDone}/6`,
    `Betrieb: ${readiness.opsDone}/6`,
    `Integrationen: ${readiness.integrationDone}/6`,
    "",
    checklistStatusLines("Preflight", [
      ["preflight", "Preflight ausgeführt"],
      ["drive", "Drive-Freigaben geprüft"],
      ["sheet", "Sheet-Freigaben geprüft"],
      ["n8n", "n8n-Zugriff geprüft"],
      ["backup", "Backup-/Loeschroutine verstanden"],
      ["upload", "erstes echtes Dokument einzeln kontrollieren"]
    ], preflight),
    "",
    checklistStatusLines("Betrieb", [
      ["sheetBackup", "Google Sheet als Excel exportiert"],
      ["n8nBackup", "n8n Workflows als JSON exportiert"],
      ["driveAccess", "Drive-Freigaben kontrolliert"],
      ["auditLog", "Letzten Upload im Audit-Log geprüft"],
      ["restoreTest", "Wiederherstellung getestet"],
      ["deleteRule", "Löschregel verstanden"]
    ], ops),
    "",
    checklistStatusLines("Integrationen", [
      ["whatsappProvider", "WhatsApp Business Provider verbunden"],
      ["whatsappTemplate", "WhatsApp Template freigegeben"],
      ["recipientOptIn", "Empfänger-Opt-in geprüft"],
      ["kiEndpoint", "KI/OCR-Endpunkt hinterlegt"],
      ["n8nCredentials", "n8n Credentials geprüft"],
      ["integrationTest", "Unkritischer Integrationstest bestanden"]
    ], integrations),
    "",
    "Hinweis: Dieser Bericht dokumentiert die lokale App-Freigabe. Drive-, Sheet-, n8n- und Provider-Freigaben muessen fachlich separat nachvollziehbar bleiben."
  ].join("\n");
  downloadTextFile(`behoerden-cockpit-freigabe-${todayValue()}.txt`, report);
  const auditItem = {
    id: `LOG-${String(state.auditLog.length + 1).padStart(3, "0")}`,
    timestamp: nowTimestamp(),
    action: "Freigabebericht exportiert",
    object: `Echtdaten-Freigabe ${readiness.totalDone}/${readiness.totalRequired}`,
    area: "Cockpit",
    details: `Status ${ready ? "Freigegeben" : "Gesperrt"}; Preflight ${readiness.preflightDone}/6, Betrieb ${readiness.opsDone}/6, Integrationen ${readiness.integrationDone}/6.`,
    source: "App",
  ...
  renderActivity();
  renderMetrics();
  persistManualEntry("audit", auditItem).then((saved) => saved && showToast("Freigabebericht im Google Sheet protokolliert"));
  showToast("Freigabebericht erstellt");
}

function prepareIntegrationUploadTest() {
  const values = readIntegrationTestRun();
  values.preparedAt = values.preparedAt || nowTimestamp();
  writeIntegrationTestRun(values);
  renderIntegrationTestRun();
  openView("check");
  focusSection("documentCheckForm");
  renderSearchCount();
  showToast("Unkritischen Testupload vorbereitet");
}

function completeIntegrationTestRun() {
  const values = readIntegrationTestRun();
  const doneCount = integrationTestDoneCount(values);
  if (doneCount < 5) {
    showToast(`Integrationstest noch offen: ${doneCount}/5 Punkten erledigt`);
    return;
  }
  const confirmed = window.confirm("Unkritischen Integrationstest als bestanden protokollieren?\n\nBitte nur bestaetigen, wenn Testupload, n8n-Lauf, KI/OCR, Sheet-Eintrag und eigene Test-Erinnerung geprueft wurden.");
  if (!confirmed) return;
  values.preparedAt = values.preparedAt || nowTimestamp();
  values.completedAt = nowTimestamp();
  values.scope = "Upload, Analyse, Sheets-Eintrag, Test-Erinnerung";
  writeIntegrationTestRun(values);

  const integrations = readIntegrationChecklist();
  integrations.integrationTest = true;
  writeIntegrationChecklist(integrations);

  const auditItem = {
    id: `LOG-${String(state.auditLog.length + 1).padStart(3, "0")}`,
    timestamp: nowTimestamp(),
    action: "Unkritischer Integrationstest protokolliert",
    object: "Produktiv-Integrationen",
    area: "Automationen",
    details: `Testupload, n8n-Lauf, KI/OCR, Google-Sheets-Eintrag und eigene Test-Erinnerung wurden als geprueft markiert.${values.resultNote ? ` Notiz: ${values.resultNote}` : ""}`,
    source: "App",
    level: "info"
  };
  state.auditLog.unshift(auditItem);
  ...
  persistManualEntry("audit", auditItem).then((saved) => saved && showToast("Integrationstest im Google Sheet protokolliert"));
  showToast("Integrationstest protokolliert");
}

function exportIntegrationTestReport() {
  const values = readIntegrationTestRun();
  const integrations = readIntegrationChecklist();
  const doneCount = integrationTestDoneCount(values);
  const completed = Boolean(values.completedAt);
  const protocolItems = [
    ["upload", "Testdokument ohne echte Gesundheitsdaten hochgeladen"],
    ["drive", "Drive-Ablage und Dateiname kontrolliert"],
    ["sheet", "Google-Sheet-Eintrag geprueft"],
    ["analysis", "KI/OCR-Ergebnis plausibel geprueft"],
    ["reminder", "Test-Erinnerung nur an eigene Nummer geprueft"]
  ];
  const resultItems = [
    ["driveResult", "Drive-Datei ist angekommen"],
    ["sheetResult", "Sheet-Zeile wurde geschrieben"],
    ["analysisResult", "Analyse/Audit wurde erstellt"],
    ["reminderResult", "Test-Erinnerung wurde kontrolliert"]
  ];
  const report = [
    "Behörden-Cockpit Integrations-Testprotokoll",
    "",
    `Erstellt: ${nowTimestamp()}`,
    `Status: ${completed ? "Bestanden" : "Offen"}`,
    `Fortschritt: ${doneCount}/5`,
    `Vorbereitet: ${values.preparedAt || "-"}`,
    `Protokolliert: ${values.completedAt || "-"}`,
    `Integrationsfreigabe: ${integrations.integrationTest ? "Bestaetigt" : "Offen"}`,
    `Testkanal: ${values.channel || "whatsapp"}`,
    `Testempfaenger: ${maskRecipient(values.recipient)}`,
    `Opt-in bestaetigt: ${values.optIn ? "Ja" : "Nein"}`,
    "",
    "Testnachricht:",
    integrationTestMessagePreview(values),
    "",
    "Pruefpunkte:",
    ...protocolItems.map(([key, label]) => `${values[key] ? "[x]" : "[ ]"} ${label}`),
    "",
    "n8n-Ergebnis:",
    ...resultItems.map(([key, label]) => `${values[key] ? "[x]" : "[ ]"} ${label}`),
    "",
    `Ergebnisnotiz: ${values.resultNote || "-"}`,
    "",
    "Hinweis: Dieses Protokoll dokumentiert nur den unkritischen Testlauf. Echte Gesundheits- oder Behoerdendaten duerfen erst nach vollstaendiger Echtdaten-Freigabe genutzt werden."
  ].join("\n");
  downloadTextFile(`behoerden-cockpit-integrationstest-${todayValue()}.txt`, report);
  const auditItem = {
    id: `LOG-${String(state.auditLog.length + 1).padStart(3, "0")}`,
    timestamp: nowTimestamp(),
    action: "Integrations-Testprotokoll exportiert",
    object: `Integrationstest ${doneCount}/5`,
    area: "Automationen",
    details: `Status ${completed ? "Bestanden" : "Offen"}; Kanal ${values.channel || "whatsapp"}; Empfaenger ${maskRecipient(values.recipient)}; Opt-in ${values.optIn ? "ja" : "nein"}.`,
    source: "App",
  ...
  renderActivity();
  renderMetrics();
  persistManualEntry("audit", auditItem).then((saved) => saved && showToast("Testprotokoll im Google Sheet protokolliert"));
  showToast("Testprotokoll erstellt");
}

async function copyIntegrationMessagePreview() {
  const values = readIntegrationTestRun();
  const message = integrationTestMessagePreview(values);
  try {
    ...
    showToast("Kopieren im Browser nicht erlaubt");
    return;
  }
  const auditItem = {
    id: `LOG-${String(state.auditLog.length + 1).padStart(3, "0")}`,
    timestamp: nowTimestamp(),
    action: "Testnachricht kopiert",
    object: values.channel || "whatsapp",
    area: "Automationen",
    details: `Empfaenger ${maskRecipient(values.recipient)}; Opt-in ${values.optIn ? "ja" : "nein"}.`,
    source: "App",
    level: "info"
  };
  state.auditLog.unshift(auditItem);
  renderActivity();
  renderMetrics();
  persistManualEntry("audit", auditItem).then((saved) => saved && showToast("Kopie im Google Sheet protokolliert"));
  showToast("Testnachricht kopiert");
}

async function copyIntegrationTestPayload() {
  const values = readIntegrationTestRun();
  const payloadText = JSON.stringify(integrationTestPayload(values), null, 2);
  try {
    if (navigator.clipboard?.writeText) {
    ...
    showToast("Payload-Kopie im Browser nicht erlaubt");
    return;
  }
  markIntegrationPayloadPrepared(values, "n8n-Testpayload kopiert", "Payload-Kopie im Google Sheet protokolliert", "n8n-Testpayload kopiert");
}

function downloadIntegrationTestPayload() {
  const values = readIntegrationTestRun();
  const payloadText = JSON.stringify(integrationTestPayload(values), null, 2);
  downloadTextFile(`behoerden-cockpit-n8n-testpayload-${todayValue()}.json`, payloadText);
  markIntegrationPayloadPrepared(values, "n8n-Testpayload-Datei erstellt", "Payload-Datei im Google Sheet protokolliert", "Payload-Datei erstellt");
}

function markIntegrationPayloadPrepared(values, action, savedToast, doneToast) {
  const auditItem = {
    id: `LOG-${String(state.auditLog.length + 1).padStart(3, "0")}`,
    timestamp: nowTimestamp(),
    action,
    object: values.channel || "whatsapp",
    area: "Automationen",
    details: `Testpayload ohne echte Gesundheitsdaten; Empfaenger ${maskRecipient(values.recipient)}.`,
    source: "App",
    level: "info"
  };
  values.preparedAt = values.preparedAt || nowTimestamp();
  ...
  renderActivity();
  renderMetrics();
  persistManualEntry("audit", auditItem).then((saved) => {
    if (saved) {
      showToast(savedToast);
      return;
    }
    const fallbackAuditItem = {
      id: `LOG-${String(state.auditLog.length + 1).padStart(3, "0")}`,
      timestamp: nowTimestamp(),
      action: "Sheet-Speicherung offen",
      object: action,
      area: "Automationen",
      details: "Der lokale Payload-Schritt wurde gespeichert, aber die Google-Sheet-Protokollierung muss spaeter erneut geprueft werden.",
      source: "App",
      level: "warning"
    };
    state.auditLog.unshift(fallbackAuditItem);
    renderActivity();
    renderMetrics();
    renderQualityStrip();
  });
...
function bindEvents() {
  ...
  if (els.integrationChecklist) {
    els.integrationChecklist.addEventListener("change", (event) => {
      const input = event.target.closest("[data-integration-item]");
      if (!input) return;
      const values = readIntegrationChecklist();
      values[input.dataset.integrationItem] = input.checked;
      writeIntegrationChecklist(values);
      renderIntegrationChecklist();
      renderIntegrationTestRun();
      renderReadinessBanner();
      renderQualityStrip();
      updateDocumentCheckStatus();
      showToast(input.checked ? "Integrationspunkt abgehakt" : "Integrationspunkt offen");
    });
  }

  document.querySelector("#openIntegrationUploadTestButton")?.addEventListener("click", prepareIntegrationUploadTest);
  document.querySelector("#markIntegrationTestButton")?.addEventListener("click", completeIntegrationTestRun);
  document.querySelector("#downloadIntegrationTestReportButton")?.addEventListener("click", exportIntegrationTestReport);
  document.querySelector("#copyIntegrationPayloadButton")?.addEventListener("click", copyIntegrationTestPayload);
  document.querySelector("#downloadIntegrationPayloadButton")?.addEventListener("click", downloadIntegrationTestPayload);
  els.integrationMessagePreview?.addEventListener("click", (event) => {
    if (!event.target.closest("#copyIntegrationMessageButton")) return;
    copyIntegrationMessagePreview();
  });

  if (els.integrationTestChecklist) {
    els.integrationTestChecklist.addEventListener("change", (event) => {
      const input = event.target.closest("[data-integration-test-item]");
      if (!input) return;
      const values = readIntegrationTestRun();
      if (input.dataset.integrationTestItem === "reminder" && !isIntegrationTestRecipientReady(values)) {
        values.reminder = false;
        writeIntegrationTestRun(values);
        renderIntegrationTestRun();
        showToast("Erst Testempfänger und Opt-in vorbereiten");
        return;
      }
      values.preparedAt = values.preparedAt || nowTimestamp();
      values[input.dataset.integrationTestItem] = input.checked;
      if (!input.checked) delete values.completedAt;
      writeIntegrationTestRun(values);
      renderIntegrationTestRun();
      showToast(input.checked ? "Testpunkt abgehakt" : "Testpunkt offen");
    });
  }

  if (els.integrationFlowGuide) {
    els.integrationFlowGuide.addEventListener("change", (event) => {
      const input = event.target.closest("[data-flow-item]");
      if (!input) return;
      const values = readIntegrationTestRun();
      values.preparedAt = values.preparedAt || nowTimestamp();
      values[input.dataset.flowItem] = input.checked;
      writeIntegrationTestRun(values);
      renderIntegrationTestRun();
      showToast(input.checked ? "n8n-Ablaufschritt abgehakt" : "n8n-Ablaufschritt offen");
    });
  }

  if (els.integrationResultCheck) {
    els.integrationResultCheck.addEventListener("change", (event) => {
      const input = event.target.closest("[data-result-item]");
      if (!input) return;
      const values = readIntegrationTestRun();
      values.preparedAt = values.preparedAt || nowTimestamp();
      values[input.dataset.resultItem] = input.checked;
      if (integrationResultDoneCount(values) === 4) values.resultsChecked = true;
      if (!input.checked) {
        values.resultsChecked = false;
        delete values.completedAt;
      }
      writeIntegrationTestRun(values);
      renderIntegrationTestRun();
      showToast(input.checked ? "n8n-Ergebnis abgehakt" : "n8n-Ergebnis offen");
    });
  }

  els.integrationResultNoteInput?.addEventListener("change", () => {
    const values = readIntegrationTestRun();
    values.preparedAt = values.preparedAt || nowTimestamp();
    values.resultNote = els.integrationResultNoteInput.value.trim();
    writeIntegrationTestRun(values);
    renderIntegrationTestRun();
    showToast(values.resultNote ? "n8n-Ergebnisnotiz gespeichert" : "n8n-Ergebnisnotiz geleert");
  });

  [els.integrationTestChannelInput, els.integrationTestRecipientInput, els.integrationTestOptInInput].forEach((input) => {
    input?.addEventListener("change", () => {
      const values = readIntegrationTestRun();
      values.preparedAt = values.preparedAt || nowTimestamp();
      values.channel = els.integrationTestChannelInput?.value || "whatsapp";
      values.recipient = values.channel === "audit" ? "" : els.integrationTestRecipientInput?.value.trim() || "";
      values.optIn = values.channel === "audit" ? true : Boolean(els.integrationTestOptInInput?.checked);
      writeIntegrationTestRun(values);
      renderIntegrationTestRun();
      showToast("Testversand-Konfiguration gespeichert");
    });
  });
  ...
  els.resetIntegrationChecklistButton?.addEventListener("click", () => {
    const confirmed = window.confirm("Integrations-Checkliste wirklich zurücksetzen?");
    if (!confirmed) return;
    removeStoredValue(INTEGRATION_CHECKLIST_STORAGE_KEY);
    removeStoredValue(INTEGRATION_TEST_STORAGE_KEY);
    renderIntegrationChecklist();
    renderIntegrationTestRun();
    renderReadinessBanner();
    renderQualityStrip();
    updateDocumentCheckStatus();
    showToast("Integrationen zurückgesetzt");
  });
  ...
  els.activityList.addEventListener("click", (event) => {
    const retryButton = event.target.closest("[data-retry-sheet-sync]");
    if (retryButton) {
      retrySheetSyncFallback(retryButton.dataset.retrySheetSync);
      return;
    }
    const button = event.target.closest("[data-resolve-sheet-sync]");
    if (!button) return;
    resolveSheetSyncFallback(button.dataset.resolveSheetSync);
  });
  ...
  els.readinessBanner.addEventListener("click", (event) => {
    if (event.target.closest("#downloadReadinessReportButton")) {
      exportReadinessReport();
      return;
    }
    const nextButton = event.target.closest("#openReadinessNextStepButton");
    if (!nextButton) return;
    openView(nextButton.dataset.readinessTarget);
    focusSection(nextButton.dataset.readinessFocus);
    renderSearchCount();
    showToast(nextButton.dataset.readinessToast || "Bereich geöffnet");
  });
  ...
  document.querySelector("#documentCheckForm").addEventListener("submit", async (event) => {
    ...
    if (file && !isFullReadinessReady()) {
      const readiness = readinessCounts();
      const confirmed = window.confirm(`Die Echtdaten-Freigabe ist noch nicht vollständig.\n\nAktueller Stand: ${readiness.totalDone}/${readiness.totalRequired}\nPreflight: ${readiness.preflightDone}/6\nBetrieb: ${readiness.opsDone}/6\nIntegrationen: ${readiness.integrationDone}/6\n\nBitte nur mit einem unkritischen Testdokument fortfahren. Upload trotzdem starten?`);
      if (!confirmed) {
        showToast("Upload abgebrochen: Echtdaten-Freigabe offen");
        return;
      }
      if (!isPreflightReady()) {
        logPreflightOverride({ fileName, area, type });
      } else {
        logReadinessOverride({ fileName, area, type });
      }
    }
    const normalizedFileName = buildDocumentFileName({ originalName: fileName, area, type });
